...
#[derive(Debug)]
enum TicketNewError {
    TitleInvalidError(String),
    DescriptionInvalidError(String),
}
...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    match Ticket::new(title.clone(), description, status.clone()) {
        Err(TicketNewError::TitleInvalidError(msg)) => {
            panic!("{}", msg)
        }
        Err(TicketNewError::DescriptionInvalidError(_)) => {
...
impl Ticket {
    pub fn new(
        ...
        if title.is_empty() {
            return Err(TicketNewError::TitleInvalidError(
        ...
        if description.is_empty() {
            return Err(TicketNewError::DescriptionInvalidError(
